<?php
	...
	class HtmlTable
	{
		...
		{		
			...
			{
				$this -> out_html .= '<th class="header h' . $j . '">' . (string) $this -> data[0][$j] . '</th>' . "\n";	
			}
			$this -> out_html .= '</thead>' . "\n";
			$this -> out_html .= '<tbody>' . "\n";
                        $this -> out_html .= '</tr>' . "\n";
		...
		public function show_list($page_name,$page_index,$pre_page)
		{
			...
			$this -> out_html .= '<span class="all_page">共' . $all_page  . '頁 </span> /' . "\n";
					...
					if($pre_page > 1) // 加上上一頁
					{
                        ...
                        $this -> out_html .=  '<a href="' .$page_name . $page_index . '=1"' . ">第一頁</a>" . "\n";
                        $this -> out_html .=  '</span>' . "\n"; 
					...
					if($all_page > $pre_page) // 加上下一頁
					{
						...
						$this -> out_html .=  '</span>' . "\n";			
                        $this -> out_html .=  '<span class="page">' . "\n";
                        $this -> out_html .=  '<a href="' . $page_name . $page_index . '=' . $all_page . '"' . ">最後頁</a>" . "\n";
                        $this -> out_html .=  '</span>' . "\n"; 
                    }
                    else
                    {
                ...
                public function PrProcess($process)
                {
                    foreach ($process as $work)
                    {
                        ...
                        {                                                             
                          ...
                          // 作函數處理
                        elseif ($work[0] == "SecondText")
                        {                                                             
                                for ($i = 1; $i < count($this->data); $i++)
                                {
                                    $ar = $this->data[$i];
                                    
                                    if(strrpos($ar[$work[2]],$work[1]))
                                    {
                                        $d = explode($work[1],$ar[$work[2]]);
